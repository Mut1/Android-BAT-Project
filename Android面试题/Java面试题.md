...
抽象类是自下而上设计的。也就是说先有子类代码的重复，然后再抽象成上层的父类（也就是抽象类）；抽象类则是更多的是模板的能力，把子类共有的抽象到父类，可以理解为自下而上的设计；

接口是自下而上设计的。一般是先考虑设计接口，再去考虑具体的实现。接口是一种规范，用好接口可以很好的进行解耦，自上而下设计可以面向接口编程；

从**含义**角度来看：抽象类表示的是 `is a` 的关系；接口表示的是 `has a` 的关系；
